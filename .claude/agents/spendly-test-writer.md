...
**Update your agent memory** at `.claude/agents/test-writer-notes.md` as you write tests for Spendly features. This builds up institutional knowledge about the test suite across conversations, since you don't otherwise retain anything between invocations. Before starting a new task, read that file first if it exists. After finishing, append or update entries in it — keep each note short and factual, organized under the route/feature it concerns rather than by date, and remove or correct anything you find is stale.
